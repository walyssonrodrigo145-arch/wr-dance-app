...
export async function runAutoMigrations() {
  ...
  try {
    ...
    const migrations = [
      ...
      { table: 'coreografias', sql: `CREATE TABLE IF NOT EXISTS "coreografias" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
        "createdByUserId" integer NOT NULL,
        "title" varchar(255) NOT NULL,
        "modalidadeId" integer,
        "nivel" varchar(30) DEFAULT 'todas' NOT NULL,
        "formacao" varchar(30) DEFAULT 'grupo' NOT NULL,
        "professorId" integer,
        "musica" varchar(255),
        "descricao" text,
        "videoUrl" text,
        "videoId" varchar(20),
        "status" varchar(30) DEFAULT 'em_montagem' NOT NULL,
        "eventId" integer,
        "active" boolean DEFAULT true NOT NULL,
        "createdAt" timestamp DEFAULT now() NOT NULL,
        "updatedAt" timestamp DEFAULT now() NOT NULL
      );` },
      { table: 'coreografias', sql: `CREATE INDEX IF NOT EXISTS "coreografias_org_idx" ON "coreografias" ("organizationId", "status")` },
      { table: 'coreografias', sql: `CREATE INDEX IF NOT EXISTS "coreografias_org_modalidade_idx" ON "coreografias" ("organizationId", "modalidadeId")` },
      { table: 'coreografia_alunos', sql: `CREATE TABLE IF NOT EXISTS "coreografia_alunos" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
        "coreografiaId" integer NOT NULL,
        "studentId" integer NOT NULL,
        "papel" varchar(120),
        "status" varchar(30) DEFAULT 'convidado' NOT NULL,
        "progresso" integer DEFAULT 0 NOT NULL,
        "observacoes" text,
        "createdAt" timestamp DEFAULT now() NOT NULL,
        "updatedAt" timestamp DEFAULT now() NOT NULL
      );` },
      { table: 'coreografia_alunos', sql: `CREATE UNIQUE INDEX IF NOT EXISTS "coreografia_alunos_unique" ON "coreografia_alunos" ("coreografiaId", "studentId")` },
      { table: 'coreografia_alunos', sql: `CREATE INDEX IF NOT EXISTS "coreografia_alunos_student_idx" ON "coreografia_alunos" ("studentId")` },
      { table: 'coreografia_alunos', sql: `CREATE INDEX IF NOT EXISTS "coreografia_alunos_org_idx" ON "coreografia_alunos" ("organizationId")` },

      // ═══ EVENTOS / ESPETÁCULOS (DancePro) ═══
      { table: 'events', sql: `CREATE TABLE IF NOT EXISTS "events" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
        "createdByUserId" integer NOT NULL,
        "name" varchar(255) NOT NULL,
        "type" varchar(30) DEFAULT 'recital' NOT NULL,
        "description" text,
        "venueName" varchar(255),
        "venueAddress" text,
        "startsAt" timestamp NOT NULL,
        "endsAt" timestamp,
        "status" varchar(30) DEFAULT 'planejado' NOT NULL,
        "requiresAuthorization" boolean DEFAULT true NOT NULL,
        "active" boolean DEFAULT true NOT NULL,
        "createdAt" timestamp DEFAULT now() NOT NULL,
        "updatedAt" timestamp DEFAULT now() NOT NULL
      );` },
      { table: 'events', sql: `CREATE INDEX IF NOT EXISTS "events_org_idx" ON "events" ("organizationId", "startsAt")` },
      { table: 'events', sql: `CREATE INDEX IF NOT EXISTS "events_org_status_idx" ON "events" ("organizationId", "status")` },
      { table: 'event_choreographies', sql: `CREATE TABLE IF NOT EXISTS "event_choreographies" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
      ...
      { table: 'event_choreographies', sql: `CREATE UNIQUE INDEX IF NOT EXISTS "event_choreographies_unique" ON "event_choreographies" ("eventId", "coreografiaId")` },
      { table: 'event_choreographies', sql: `CREATE INDEX IF NOT EXISTS "event_choreographies_event_idx" ON "event_choreographies" ("eventId")` },
      { table: 'event_participants', sql: `CREATE TABLE IF NOT EXISTS "event_participants" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
        "eventId" integer NOT NULL,
        "studentId" integer NOT NULL,
        "status" varchar(30) DEFAULT 'convidado' NOT NULL,
        "imageAuthorization" boolean DEFAULT false NOT NULL,
        "participationAuthorization" boolean DEFAULT false NOT NULL,
      ...
      { table: 'event_participants', sql: `CREATE UNIQUE INDEX IF NOT EXISTS "event_participants_unique" ON "event_participants" ("eventId", "studentId")` },
      { table: 'event_participants', sql: `CREATE INDEX IF NOT EXISTS "event_participants_student_idx" ON "event_participants" ("studentId")` },
      { table: 'event_participants', sql: `CREATE INDEX IF NOT EXISTS "event_participants_org_idx" ON "event_participants" ("organizationId")` },

      // ═══ FIGURINOS / VESTUÁRIO (DancePro) ═══
      { table: 'costumes', sql: `CREATE TABLE IF NOT EXISTS "costumes" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
        "createdByUserId" integer NOT NULL,
        "name" varchar(255) NOT NULL,
        "code" varchar(60),
        "type" varchar(60) DEFAULT 'outro' NOT NULL,
        "size" varchar(30),
        "color" varchar(60),
        "quantity" integer DEFAULT 1 NOT NULL,
        "condition" varchar(30) DEFAULT 'bom' NOT NULL,
      ...
      { table: 'costumes', sql: `CREATE INDEX IF NOT EXISTS "costumes_org_idx" ON "costumes" ("organizationId", "active")` },
      { table: 'costumes', sql: `CREATE INDEX IF NOT EXISTS "costumes_org_name_idx" ON "costumes" ("organizationId", "name")` },
      { table: 'costume_loans', sql: `CREATE TABLE IF NOT EXISTS "costume_loans" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
        "createdByUserId" integer NOT NULL,
        "costumeId" integer NOT NULL,
        "studentId" integer NOT NULL,
        "coreografiaId" integer,
        "eventId" integer,
        "quantity" integer DEFAULT 1 NOT NULL,
        "checkedOutAt" timestamp DEFAULT now() NOT NULL,
        "dueDate" date,
        "returnedAt" timestamp,
        "conditionOnReturn" varchar(30),
        "notes" text,
        "createdAt" timestamp DEFAULT now() NOT NULL,
        "updatedAt" timestamp DEFAULT now() NOT NULL
      );` },
      { table: 'costume_loans', sql: `CREATE INDEX IF NOT EXISTS "costume_loans_costume_idx" ON "costume_loans" ("costumeId")` },
      { table: 'costume_loans', sql: `CREATE INDEX IF NOT EXISTS "costume_loans_student_idx" ON "costume_loans" ("studentId")` },
      { table: 'costume_loans', sql: `CREATE INDEX IF NOT EXISTS "costume_loans_org_idx" ON "costume_loans" ("organizationId", "returnedAt")` },

      // ═══ TURMAS COM VAGAS E LISTA DE ESPERA (DancePro) ═══
      { table: 'turmas', sql: `CREATE TABLE IF NOT EXISTS "turmas" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
        "createdByUserId" integer NOT NULL,
        "name" varchar(255) NOT NULL,
        ...
        "level" varchar(30) DEFAULT 'todas' NOT NULL,
        "status" varchar(30) DEFAULT 'ativa' NOT NULL,
        "notes" text,
        "createdAt" timestamp DEFAULT now() NOT NULL,
        "updatedAt" timestamp DEFAULT now() NOT NULL
      );` },
      { table: 'turmas', sql: `CREATE INDEX IF NOT EXISTS "turmas_org_idx" ON "turmas" ("organizationId", "status")` },
      { table: 'turmas', sql: `CREATE INDEX IF NOT EXISTS "turmas_org_modalidade_idx" ON "turmas" ("organizationId", "modalidadeId")` },
      { table: 'turma_alunos', sql: `CREATE TABLE IF NOT EXISTS "turma_alunos" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
      ...
      { table: 'turma_alunos', sql: `CREATE UNIQUE INDEX IF NOT EXISTS "turma_alunos_unique" ON "turma_alunos" ("turmaId", "studentId")` },
      ...
      { table: 'student_health_records', sql: `CREATE TABLE IF NOT EXISTS "student_health_records" (
        ...
        "heightCm" integer,
        "flexibilityCm" decimal(5,1),
        "conditioning" varchar(20),
        "injuryNotes" text,
        "restrictions" text,
        "notes" text,
        "createdAt" timestamp DEFAULT now() NOT NULL
      );` },
      { table: 'student_health_records', sql: `CREATE INDEX IF NOT EXISTS "student_health_records_student_idx" ON "student_health_records" ("studentId", "recordDate")` },
      { table: 'student_health_records', sql: `CREATE INDEX IF NOT EXISTS "student_health_records_org_idx" ON "student_health_records" ("organizationId")` },

      // ═══ NPS — PESQUISA DE SATISFAÇÃO (DancePro) ═══
      { table: 'nps_responses', sql: `CREATE TABLE IF NOT EXISTS "nps_responses" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL,
        "studentId" integer,
        "score" integer NOT NULL,
        "comment" text,
        "source" varchar(30) DEFAULT 'portal' NOT NULL,
        "createdByUserId" integer,

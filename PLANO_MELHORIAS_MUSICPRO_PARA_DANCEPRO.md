...
- ✅ **2.2 Gestão de alunos na sessão concluída**: `lesson_overrides` + `turmaAttendance.addStudent/removeStudent` (extra nesta/próximas; remoção só desta aula ou das próximas) com badge Extra e auditoria; chamada respeita as exceções.
- ✅ **2.3 Agendamento visível (parte 1) concluída**: prévia no cadastro com grade das turmas, estimativa de aulas e mensalidades ao salvar.
- ✅ **3.1 Meu Perfil do professor concluída**: página `/perfil` (foto, dados, PIX, apresentação e troca de senha com senha atual); `settings.getMyProfile/updateMyAvatar` + `auth.updateMyPassword` exigindo a senha atual. Atalho no menu do usuário.
- ✅ **3.2 Boot loader + LoadingScreen concluída**: boot loader no `index.html` (fim do flash branco) + `LoadingScreen` unificado nas rotas.
...
- ⏭️ Próximas etapas: **1.2 SEO público** → **2.1 Indique e Ganhe** → **2.4** cobrança boleto/link → **3.3** contratos em blocos → **3.4** fatura avulsa/simulador → **3.5** Diário de ensaio.
